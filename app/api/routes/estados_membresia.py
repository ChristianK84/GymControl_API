...
def list_estados(db: Session = Depends(get_db), _current=Depends(get_current_user)):
...
def get_estado(estado_id: int, db: Session = Depends(get_db), _current=Depends(get_current_user)):
